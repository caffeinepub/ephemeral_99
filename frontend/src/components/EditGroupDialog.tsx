...
import { useFriends, useUpdateGroup } from "../hooks/useQueries";
...
export function EditGroupDialog({
  ...
  useEffect(() => {
    if (open) {
      setName(initialName);
      setSelectedMembers(
        new Set(initialMembers.map((m) => m.principal.toString())),
      );
      setError("");
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updateGroup(
      { groupId, name: name.trim(), members: memberPrincipals },
      {
        onSuccess: () => {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <DialogFooter>
            <Button type="submit" disabled={isPending}>
              ...
              {isPending ? "Saving..." : `Save (${selectedMembers.size})`}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
